...
| Expiry / utilization-threshold alerts (sections 47-48) | EXISTS | `alertsService.ts` -- computed on demand plus a daily cron (`/api/cron/license-alerts`); `deliverLicenseAlerts` queues a durable `ComplianceNotification` digest (type `LICENSE_ALERT`) through `ComplianceNotificationDispatcher` rather than sending email inline. |
...
| Unified `ComplianceExecution` audit envelope integration (Audit/Service Usage/Compliance History prompt) | EXISTS | `ComplianceExecutionType.LICENSE_DETERMINATION`/`IMPORT_CONTROL_DETERMINATION` already recorded by `determinationService.runLicenseDetermination` -- `resultRefType: "LicenseDeterminationResult"`/`resultRefId` linkage, sanitized request/response snapshots, `ComplianceFormalOverride` and `/api/v1/compliance/executions/[id]` drill-down all work generically (domain-agnostic, no code change needed). |
| Audit & History UI -- execution-type filter includes License | FIXED (was MISSING) | `ExecutionHistoryPanel.tsx`'s type-filter `<select>` hardcoded 8 screening types and omitted `LICENSE_DETERMINATION`/`IMPORT_CONTROL_DETERMINATION` -- license executions were recorded and drillable by ID, but not selectable from the Audit & History type dropdown. Added both values to the option list. |
| Formal override create/revoke UI (Audit/Service Usage/Compliance History prompt) | FIXED (was MISSING) | `POST /api/v1/compliance/overrides` and `.../overrides/[id]/revoke` existed with no UI anywhere in the app. Added a create-override form and per-active-override revoke control to `ExecutionHistoryPanel.tsx`'s execution detail view, gated by a new `mayCreateFormalOverride` prop threaded from `compliance/page.tsx` (`holdsPermission(context, "compliance.override")`). Domain-agnostic (applies to every `ComplianceExecution` type, not License-specific), but surfaced during the License-focused audit review. |
| Email notifications for License Determination review + portfolio alerts | EXISTS | `licenseEligibility.ts` decides eligibility (`REVIEW_REQUIRED`/`BLOCKED` outcomes; `licenseDeterminationEnabled`/`licenseManagementEnabled` + configured recipients); `licenseNotificationService.ts` queues a durable `ComplianceNotification` (types `LICENSE_DETERMINATION_REVIEW_REQUIRED`, `LICENSE_ALERT`) with a snapshotted `payload`, never sent synchronously; `dispatcher.ts` renders and sends via the same outbox pipeline RPS uses. `runLicenseDetermination` queues the review notification best-effort (try/catch, never blocks the determination). |
