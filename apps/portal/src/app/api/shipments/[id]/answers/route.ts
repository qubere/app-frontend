...
export async function GET(_req:Request,{params}:{params:Promise<{id:string}>}) {
 const ctx=await getAccountContext();if(!ctx)return NextResponse.json({error:'UNAUTHENTICATED'},{status:401});
 return portalData(ctx,async()=>{
 const {id}=await params;
 const resource=await db.shipment.findFirst({where:{id,accountId:ctx.accountId,deletedAt:null},select:{accountId:true,clientId:true,importerName:true}});if(!resource)return notFound();
 const auth=await authorizePortalResource({permission:'portal.shipments.read',resourceAccountId:resource.accountId,resourceClientId:resource.clientId,importerName:resource.importerName});if(!auth.authorized)return auth.errorResponse??notFound();
 const s=await db.shipment.findUnique({where:{id},include:{
  ...
  invoiceLines:{include:{invoice:{select:{id:true,invoiceNumber:true,status:true,totalAmount:true,currency:true}}}},
