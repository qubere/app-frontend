...
export async function POST(req: Request, { params }: {
    ...
    const { proof, auth } = result;
    const { lineNumber, body } = parsed.data;
    if (lineNumber && !(proof.payload as unknown as EntryProofPayload).lines.some(l => l.lineNumber === lineNumber))
    ...
    const request = await portalData(auth.ctx!, () => db.$transaction(async (tx) => {
        ...
        await tx.entryProofEvent.create({ data: { entryProofId: proof.id, accountId: proof.accountId, type: 'CUSTOMER_QUESTION', actorType: 'CUSTOMER', actorUserId: auth.ctx!.userId, detail: { requestId: r.id, ...(lineNumber ? { lineNumber } : {}) } } });
        return r;
    }));
