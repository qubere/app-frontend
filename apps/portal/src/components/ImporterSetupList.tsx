...
export function ImporterSetupList({ importers }: { importers: ImporterSetup[] }) {
  return <section aria-labelledby="importers-heading" className="space-y-4">
    ...
    {!importers.length && <p className="rounded-2xl border border-slate-200 bg-white p-6 text-sm text-slate-500">No importer records are linked to this client yet.</p>}
    {importers.map(i => <article key={i.id} aria-labelledby={`importer-${i.id}`} className="rounded-2xl border border-slate-200 bg-white p-6">
      ...
      <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-4 mt-5">
        ...
        <div><h4 className="font-medium text-sm mb-2">Power of Attorney</h4><dl>{field('Status', displayStatus(i.poa?.status))}{field('Signer', i.poa?.signerName)}{field('Execution method', i.poa?.executionMethod)}{field('Signed date', i.poa?.signedDate ? new Date(i.poa.signedDate).toLocaleDateString() : null)}</dl>{i.poa?.documentId && <a className="inline-block mt-2 text-sm text-[#0071E3]" href={`/api/setup/documents/${i.poa.documentId}/download`}>View signed POA →</a>}</div>
