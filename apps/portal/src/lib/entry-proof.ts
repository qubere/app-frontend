...
export async function authorizedProof(id:string,permission='portal.entries.read') {
 const filing=await db.customsFiling.findUnique({where:{id},select:{id:true,accountId:true,shipmentId:true,customerVisibleAt:true,shipment:{select:{clientId:true,importerName:true}}}});
 if(!filing) return {error:notFound()} as const;
 ...
 if(!auth.authorized || !auth.ctx || !auth.effectiveClientId) return {error:auth.errorResponse ?? notFound()} as const;
 ...
 if(!proof) return {error:notFound()} as const;
