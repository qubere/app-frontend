...
export async function portalScope(req: Request, permission: string) {
 ...
 if(!ctx) return {error:NextResponse.json({error:'UNAUTHENTICATED'},{status:401})} as const;
 if(!ctx.isPlatformAdmin && !ctx.roleNames?.some(r=>['OWNER','ADMIN','BROKER_ADMIN'].includes(r)) && !ctx.permissions?.includes(permission)) return {error:notFound()} as const;
 ...
 if(resolved.forbidden) return {error:notFound()} as const;
 return {ctx,clientIds:resolved.clientIds} as const;
}
export async function portalData<T>(ctx:{dataMode?:string|null},fn:()=>Promise<T>):Promise<T> {
