...
const m=vi.hoisted(()=>({ctx:{accountId:'a1',userId:'u1',roleNames:['CUSTOMER_USER'],permissions:['portal.shipments.read'],dataMode:'DEMO'},scope:{isAllClients:false,authorizedClientIds:['target'],teamIds:[]},db:{$queryRaw:vi.fn(),invoice:{findMany:vi.fn()},shipment:{findFirst:vi.fn(),findUnique:vi.fn()}}}));
...
describe('Published proof cost summaries', () => {
 it('returns authoritative totals without transferring proof JSON and keeps partial costs marked', async () => {
  ...
  expect(values).toEqual(['a1', 'DEMO', 'target', 's1', 's1']);
 ...
 it('never runs proof SQL for another client', async () => {
  m.ctx.permissions.push('portal.entries.read');
  m.db.shipment.findFirst.mockResolvedValue({ accountId: 'a1', clientId: 'amazon', importerName: 'Amazon' });
