...
const m = vi.hoisted(() => ({
  ctx: { accountId: 'broker', userId: 'porter', dataMode: 'DEMO', roleNames: ['CUSTOMER_ADMIN'], permissions: ['portal.shipments.read', 'portal.documents.read', 'portal.setup.read'] },
...
const importer = (id: string, clientId: string | null = 'target') => ({ id, accountId: 'broker', clientId, onboardingEntities: [] });
beforeEach(() => {
  vi.resetAllMocks(); m.ctx.permissions = ['portal.shipments.read', 'portal.documents.read', 'portal.setup.read'];
  ...
  m.db.shipmentDocument.findUnique.mockResolvedValue({ id: 'doc', accountId: 'broker', clientId: null, portalVisibility: 'CUSTOMER', fileUrl: 'stored://invoice', fileName: 'Invoice.pdf', shipment: { accountId: 'broker', clientId: null, importerOfRecordId: 'ior-one', deletedAt: null } });
  m.read.mockResolvedValue({ body: Buffer.from('%PDF-1.7 test') });
});
describe('Workspace ownership across importers', () => {
  it('includes direct client shipments and shipments from every authorized importer', async () => {
    ...
    expect(m.db.shipment.findMany).toHaveBeenCalledWith(expect.objectContaining({ where: { accountId: 'broker', deletedAt: null, OR: [{ clientId: { in: ['target'] } }, { clientId: null, importerOfRecordId: { in: ['ior-one', 'ior-two'] } }] } }));
  ...
  it('resolves legacy importer ownership only from an unambiguous case link', async () => {
    const legacy = { ...importer('legacy', null), onboardingEntities: [{ case: { accountId: 'broker', clientId: 'target' } }] };
    expect(importerClientId(legacy, 'broker')).toBe('target');
    legacy.onboardingEntities.push({ case: { accountId: 'broker', clientId: 'amazon' } });
    expect(importerClientId(legacy, 'broker')).toBeNull();
  });
  it('never replaces an explicit different client with an importer link', async () => {
    expect(await shipmentClientId('broker', { clientId: 'amazon', importerOfRecordId: 'ior-one' })).toBe('amazon');
    expect(m.db.importerOfRecord.findFirst).not.toHaveBeenCalled();
  });
  it('excludes unknown and other-client importers even if returned by a stale query', async () => {
    ...
    expect(m.db.shipment.findMany.mock.calls[0][0].where.OR[1].importerOfRecordId.in).toEqual([]);
  });
  it('rejects a different client filter before loading relationships', async () => {
...
describe('Shared documents and upload attribution', () => {
  it('lists shipment and setup documents with real upload attribution', async () => {
    ...
    expect(query.where.portalVisibility).toBe('CUSTOMER');
    expect(query.where.AND[0]).toMatchObject({ accountId: 'broker', AND: [{ OR: [{ clientId: { in: ['target'] } }, { clientId: null, shipment: expect.objectContaining({ accountId: 'broker', deletedAt: null }) }] }, expect.any(Object)] });
  ...
  it('uses the original inbound sender rather than an assignee', async () => {
    ...
    m.db.shipmentDocument.findMany.mockResolvedValue([{ ...record, inboundAttachment: { inboundEmail: { accountId: 'broker', normalizedFromAddress: 'supplier@example.com' } } }]);
  ...
  it('uses current assignments on every read rather than a user-only cache', async () => {
    await documents.GET(request('documents'));
    m.scope.authorizedClientIds = [];
    await documents.GET(request('documents'));
    expect(m.db.importerOfRecord.findMany).toHaveBeenCalledTimes(2);
    expect(m.db.clientDocument.findMany.mock.calls.at(-1)![0].where.clientId.in).toEqual([]);
  });
  ...
  it('denies internal or differently-owned documents before storage access', async () => {
    ...
    m.db.shipmentDocument.findUnique.mockResolvedValue({ ...record, clientId: 'amazon' });
    expect((await download.GET(request('documents/doc/download'), { params: Promise.resolve({ id: 'doc' }) })).status).toBe(404);
