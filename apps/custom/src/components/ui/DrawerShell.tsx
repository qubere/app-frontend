...
export function DrawerShell({ open, title, onClose, children, footer, busy = false }: {
  open: boolean; title: string; onClose: () => void; children: ReactNode; footer?: ReactNode; busy?: boolean;
}) {
  const titleId = useId();
    ...
    <div className="fixed inset-0 z-[100] flex justify-end bg-black/25">
      <div ref={ref} {...dialogSurfaceProps(titleId)} className="flex h-[100dvh] w-full max-w-2xl flex-col border-l border-border bg-white shadow-2xl outline-none">
        <header className="flex shrink-0 items-center justify-between border-b border-border px-6 py-4">
          ...
          <button type="button" aria-label="Save and close drawer" disabled={busy} onClick={onClose} className="rounded-lg p-2 hover:bg-gray-100 disabled:opacity-40"><X size={20}/></button>
        </header>
