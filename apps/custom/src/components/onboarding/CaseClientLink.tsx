...
export function CaseClientLink({ caseId, client, onSaved }: { caseId: string; client: ClientOption | null; onSaved: () => Promise<void> }) {
  ...
  return <section className="border-b px-6 py-3 bg-white text-sm">
    <div className="flex items-center gap-3"><span>Client: <strong>{client?.name ?? 'Not linked'}</strong></span><button className="text-brand hover:underline" onClick={() => { setSelected(client); setOpen(!open); }} disabled={busy}>Update client link</button></div>
    {message && <p role="status" className="mt-2 text-emerald-700">{message}</p>}
    {open && <div className="max-w-lg mt-4 space-y-3">
      <p className="text-ink-muted">Choose the client record for this onboarding case. Workspace members can view its setup and signed documents in the portal.</p>
      ...
      <div className="flex gap-2"><Button disabled={!selected || busy} onClick={async () => {
        ...
        try {
          ...
          await onSaved(); setOpen(false); setMessage(`Client link saved as ${selected.name}.`);
