...
export async function summarizeInboundReceipt(accountId: string, emailId: string) {
  const email = await db.inboundEmail.findFirst({ where: { id: emailId, accountId }, include: { client: { select: { name: true } }, attachments: { include: { shipmentDocument: { select: { clientId: true, shipmentId: true, portalVisibility: true, status: true, shipment: { select: { shipmentNumber: true } } } } } }, reviews: { where: { status: 'OPEN' }, select: { id: true } } } });
  ...
  const visible = docs.filter(d => d.portalVisibility === 'CUSTOMER' && d.clientId === email.clientId);
  ...
  const members = await db.accountMembership.findMany({ where: { accountId, status: 'ACTIVE', deletedAt: null, user: { deletedAt: null, clientAssignments: { some: { clientId: email.clientId } } }, roles: { some: { role: { rolePermissions: { some: { permission: { name: 'portal.documents.read' } } } } } } }, select: { userId: true } });
