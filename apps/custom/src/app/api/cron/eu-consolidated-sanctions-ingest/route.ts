...
import { EuConsolidatedSanctionsIngestionService } from "@/modules/screening/euConsolidatedSanctionsIngestionService";

export const maxDuration = 120;

const DATASET_ID = "eu-consolidated-sanctions";
const DATASET_NAME = "EU Consolidated Financial Sanctions List";
...
async function handleIngest(requestId: string) {
      ...
      return NextResponse.json(
        {
          ...
          requestId,
          note: `EU Consolidated Sanctions List ingestion already has a run in progress (started ${alreadyRunning.startedAt.toISOString()}).`,
        },
        { status: 409 }
      );
    }
  ...
  try {
    const result = await EuConsolidatedSanctionsIngestionService.fetchAndIngest();
    await db.datasetRefreshLog.update({
      where: { id: log.id },
      data: {
        status: "SUCCESS",
    ...
    const errorMessage = err.message || "EU Consolidated Sanctions List ingestion failed";
    await db.datasetRefreshLog.update({
      where: { id: log.id },
      data: { status: "FAILED", errorMessage, completedAt: new Date() },
    });
    console.error("[eu-consolidated-sanctions-ingest] Execution failed:", err);
