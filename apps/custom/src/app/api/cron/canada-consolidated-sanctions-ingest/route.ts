...
import { CanadaConsolidatedSanctionsListIngestionService } from "@/modules/screening/canadaConsolidatedSanctionsListIngestionService";

export const maxDuration = 120;

const DATASET_ID = "canada-consolidated-sanctions-list";
const DATASET_NAME = "Consolidated Canadian Autonomous Sanctions List";
...
async function handleIngest(requestId: string) {
      ...
      return NextResponse.json(
        {
          ...
          requestId,
          note: `Canada Consolidated Sanctions List ingestion already has a run in progress (started ${alreadyRunning.startedAt.toISOString()}).`,
        },
        { status: 409 }
      );
    }
  ...
  try {
    const result = await CanadaConsolidatedSanctionsListIngestionService.fetchAndIngest();
    await db.datasetRefreshLog.update({
      where: { id: log.id },
      data: {
        status: "SUCCESS",
    ...
    const errorMessage = err.message || "Canada Consolidated Sanctions List ingestion failed";
    await db.datasetRefreshLog.update({
      where: { id: log.id },
      data: { status: "FAILED", errorMessage, completedAt: new Date() },
    });
    console.error("[canada-consolidated-sanctions-ingest] Execution failed:", err);
