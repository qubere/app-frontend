...
import { MasDomesticDesignationsIngestionService } from "@/modules/screening/masDomesticDesignationsIngestionService";

export const maxDuration = 60;

const DATASET_ID = "mas-domestic-designations";
const DATASET_NAME = "MAS Domestic Designations (TSFA2002 First Schedule)";
...
async function handleIngest(requestId: string) {
      ...
      return NextResponse.json(
        {
          ...
          requestId,
          note: `MAS domestic designations ingestion already has a run in progress (started ${alreadyRunning.startedAt.toISOString()}).`,
        },
        { status: 409 }
      );
    }
  ...
  try {
    const result = await MasDomesticDesignationsIngestionService.fetchAndIngest();
    await db.datasetRefreshLog.update({
      where: { id: log.id },
      data: {
        status: "SUCCESS",
    ...
    const errorMessage = err.message || "MAS domestic designations ingestion failed";
    await db.datasetRefreshLog.update({
      where: { id: log.id },
      data: { status: "FAILED", errorMessage, completedAt: new Date() },
    });
    console.error("[mas-domestic-designations-ingest] Execution failed:", err);
