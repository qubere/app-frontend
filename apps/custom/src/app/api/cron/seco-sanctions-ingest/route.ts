...
import { SecoSanctionsListIngestionService } from "@/modules/screening/secoSanctionsListIngestionService";

export const maxDuration = 120;

const DATASET_ID = "seco-sanctions-list";
const DATASET_NAME = "SECO Sanctions List (Switzerland)";
...
async function handleIngest(requestId: string) {
      ...
      return NextResponse.json(
        {
          ...
          requestId,
          note: `SECO Sanctions List ingestion already has a run in progress (started ${alreadyRunning.startedAt.toISOString()}).`,
        },
        { status: 409 }
      );
    }
  ...
  try {
    const result = await SecoSanctionsListIngestionService.fetchAndIngest();
    await db.datasetRefreshLog.update({
      where: { id: log.id },
      data: {
        status: "SUCCESS",
        summary: `Parsed ${result.parsedCount} active targets, ${result.supersededCount} superseded. Source list date: ${result.listDate ?? "unknown"}.`,
    ...
    const errorMessage = err.message || "SECO Sanctions List ingestion failed";
    await db.datasetRefreshLog.update({
      where: { id: log.id },
      data: { status: "FAILED", errorMessage, completedAt: new Date() },
    });
    console.error("[seco-sanctions-ingest] Execution failed:", err);
