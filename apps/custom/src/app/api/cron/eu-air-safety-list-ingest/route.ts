...
import { EuAirSafetyListIngestionService } from "@/modules/screening/euAirSafetyListIngestionService";

export const maxDuration = 120;

const DATASET_ID = "eu-air-safety-list";
const DATASET_NAME = "EU Air Safety List";
...
async function handleIngest(requestId: string) {
      ...
      return NextResponse.json(
        {
          ...
          requestId,
          note: `EU Air Safety List ingestion already has a run in progress (started ${alreadyRunning.startedAt.toISOString()}).`,
        },
        { status: 409 }
      );
    }
  ...
  try {
    const result = await EuAirSafetyListIngestionService.fetchAndIngest();
    await db.datasetRefreshLog.update({
      where: { id: log.id },
      data: {
        status: "SUCCESS",
    ...
    const errorMessage = err.message || "EU Air Safety List ingestion failed";
    await db.datasetRefreshLog.update({
      where: { id: log.id },
      data: { status: "FAILED", errorMessage, completedAt: new Date() },
    });
    console.error("[eu-air-safety-list-ingest] Execution failed:", err);
