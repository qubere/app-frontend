...
export const GET = withAuthenticatedRoute(async ({ req, ctx }) => {
  ...
  if (q.get("includeClosed") === "true" && !q.get("shipmentId")) throw new DomainError("Choose a shipment to review closed holds.", "SHIPMENT_REQUIRED", 400);
  ...
  return NextResponse.json(await listHolds(ctx.accountId, { shipmentId: q.get("shipmentId") || undefined, agency: q.get("agency") || undefined, importer: q.get("importer")?.slice(0,200) || undefined, page, oldestFirst: q.get("sort") !== "newest", includeClosed: q.get("includeClosed") === "true" }));
}, { permission: "pga.read" });
