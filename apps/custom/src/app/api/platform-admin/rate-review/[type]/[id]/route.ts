...
import {
  ...
  RATE_REVIEW_TYPES,
  type RateReviewType,
} from "@/modules/tradeRate/tradeRateReviewService";

type Params = { type: string; id: string };

const pathSchema = z.object({
  type: z.string().refine(isRateReviewType, {
    message: `type must be one of: ${RATE_REVIEW_TYPES.join(", ")}`,
  }),
  id: z.string().min(1),
});
