/** POST /api/compliance/batches/:id/rescreen -- re-runs screening on every valid record of a COMPLETED/FAILED batch. */
...
export const POST = withAuthenticatedRoute<{ id: string }>(
  async ({ params, ctx, requestId }) => {
    try {
      const batch = await ComplianceBatchService.rescreenBatch(ctx.accountId, params.id, ctx.userId ?? null, requestId);
      if (!batch) {
  ...
  { permission: "compliance.bulk_screening.rescreen", write: true }
);
