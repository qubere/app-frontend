...
export const GET = withAuthenticatedRoute<{ id: string }>(async ({ ctx, params, req }) => {
  const review = await db.inboundDocumentReview.findFirst({ where: { id: params.id, accountId: ctx.accountId }, select: { clientId: true, candidateSummary: true } });
  if (!review) return NextResponse.json({ error: 'NOT_FOUND' }, { status: 404 });
  const q = new URL(req.url).searchParams.get('q')?.slice(0, 100) || '';
  const candidateIds = Array.isArray(review.candidateSummary) ? review.candidateSummary.flatMap(c => c && typeof c === 'object' && !Array.isArray(c) && typeof c.shipmentId === 'string' ? [c.shipmentId] : []).slice(0, 50) : [];
