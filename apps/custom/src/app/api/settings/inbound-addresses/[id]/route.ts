...
const schema = z.object({ action: z.enum(['SUSPEND', 'RESUME', 'REVOKE', 'ROTATE', 'POLICY']), senderPolicy: z.enum(['OPEN', 'ALLOWLIST', 'REVIEW']).optional(), autoReplyEnabled: z.boolean().optional(), autoAttachPolicy: z.enum(['OFF', 'CONFIDENT', 'AGGRESSIVE']).optional() });
export const PATCH = withAuthenticatedRoute<{ id: string }>(async ({ ctx, params, req, requestId }) => {
  ...
  try { return NextResponse.json({ address: await changeInboundAddress(ctx.accountId, params.id, body.data.action, ctx.userId, { senderPolicy: body.data.senderPolicy, autoReplyEnabled: body.data.autoReplyEnabled, autoAttachPolicy: body.data.autoAttachPolicy }) }); }
