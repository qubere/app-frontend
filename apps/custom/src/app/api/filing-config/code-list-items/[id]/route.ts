...
import { updateItem, deleteItem, itemUpdateSchema, DomainCodeListError } from "@/modules/filingConfig/codeListService";
...
export const PATCH = withAuthenticatedRoute<Params>(
  async ({ req, ctx, params, requestId }) => {
    ...
    const body = await parseAndValidateBody(req, itemUpdateSchema, requestId);
    if ("response" in body) return body.response;

    try {
      const row = await updateItem(params.id, body.data, ctx.userId ?? "system");
