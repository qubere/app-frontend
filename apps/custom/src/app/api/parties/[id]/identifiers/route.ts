...
import { partyIdParamSchema, partyIdentifierInputSchema } from "@/modules/party/partySchemas";
import { addIdentifier } from "@/modules/party/partyService";
...
export const POST = withAuthenticatedRoute<Params>(
  async ({ req, ctx, params, requestId }) => {
    ...
    const body = await parseAndValidateBody(req, partyIdentifierInputSchema, requestId);
    if ("response" in body) return body.response;

    const outcome = await addIdentifier(partyActor(ctx, requestId), path.data.id, body.data);
