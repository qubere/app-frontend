...
import { removeAttribute } from "@/modules/product/productService";

const paramsSchema = z.object({
  id: z.string().trim().min(1).max(64),
  attributeId: z.string().trim().min(1).max(64),
});
...
export const DELETE = withAuthenticatedRoute<Params>(
  async ({ ctx, params, requestId }) => {
    ...
    const outcome = await removeAttribute(
      productActor(ctx, requestId),
      path.data.id,
      path.data.attributeId
    );
