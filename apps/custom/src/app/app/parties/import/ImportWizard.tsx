...
const OUTCOME_COPY: Record<Outcome, { label: string; className: string; hint: string }> = {
  CREATED: {
    ...
    hint: "Nothing in the party master matched, so this row becomes a new party.",
  },
  ALREADY_PRESENT: {
    label: "Already here",
    className: "bg-slate-100 text-slate-700 border-slate-200",
    hint: "An identifier or registration on this row already resolves to a party. It is skipped, so re-uploading the same file creates nothing.",
  },
  NEEDS_REVIEW: {
    label: "Needs a person",
    className: "bg-amber-50 text-amber-900 border-amber-200",
    hint: "The match was possible or ambiguous, not certain. Qubere will not guess which party this is or merge two candidates, so the row is skipped until you say.",
  },
  INVALID: {
...
export function ImportWizard() {
  ...
  async function onFile(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const response = await fetch("/api/parties/import/preview", {
        method: "POST",
  ...
  async function onCommit() {
    ...
    try {
      const response = await fetch("/api/parties/import/commit", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          content,
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-[#6E6E73] mt-1 max-w-2xl">
              A CSV with one party per row. Column names are matched case- and punctuation-insensitively;
              anything Qubere does not recognise is listed back to you rather than silently dropped.
            </p>
          </div>
          {/*
          ...
          */}
          <a
            href="/api/parties/import/template"
            download
        ...
        <>
          <section className="rounded-2xl bg-white border border-border p-5 space-y-4">
            ...
            {preview.unmappedHeaders.length > 0 && (
              <p className="text-xs text-amber-800 bg-amber-50 border border-amber-200 rounded-xl p-3">
                These columns were not recognised and will be ignored: {preview.unmappedHeaders.join(", ")}.
              </p>
            )}
          </section>
              ...
              <table className="w-full text-sm">
                <thead className="text-left text-xs font-semibold uppercase tracking-wider text-ink-muted bg-surface-muted">
                  <tr>
                    ...
                    <th className="px-3 py-3">Internal code</th>
                ...
                <tbody className="divide-y divide-border">
                  {preview.rows.map((row) => (
                    <tr key={row.rowNumber}>
                      ...
                      <td className="px-3 py-3 text-ink">{row.legalName ?? "—"}</td>
                      <td className="px-3 py-3 text-[#6E6E73]">{row.internalPartyCode ?? "—"}</td>
                      <td className="px-3 py-3">
                      ...
                      <td className="px-3 py-3 text-xs text-[#6E6E73] max-w-md">
                        ...
                        {row.matchedPartyId !== null && (
                          <Link
                            href={`/app/parties/${row.matchedPartyId}`}
                            className="font-semibold text-brand"
                          >
                            Open the matched party
                          </Link>
                        )}
          ...
          <section className="rounded-2xl bg-white border border-border p-5 space-y-3">
            ...
            <p className="text-xs text-[#6E6E73] max-w-2xl">
              Each row is written on its own, so one row failing leaves the rest imported. A row is
              never merged into an existing party by this step — only created new or left for a
              person to resolve.
            </p>
            <button
              type="button"
            ...
            >
              {busy
                ...
                : creatable === 0
                  ...
                  : `Import ${creatable} ${creatable === 1 ? "party" : "parties"}`}
            </button>
          </section>
        </>
      )}
