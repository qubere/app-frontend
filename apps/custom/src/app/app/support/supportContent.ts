...
},
{
  ...
  },
  {
    ...
    answer: "When client document addresses are enabled, the destination identifies the client. Clean attachments enter processing; a single confident shipment match can attach within that client. Unknown senders and uncertain matches need a broker decision. Email receipt is not approval for filing.",
    steps: [
      "Open Manage Account from the profile menu, then Document Email.",
      "Copy the current address labeled for the client. Customers can also copy their assigned client's address from portal Setup or Documents.",
      "Send or forward the trade document with the shipment reference in the subject or message when available.",
      "Check Documents for the file and Email review for sender or shipment decisions. Approved senders only holds unknown mail before attachment download.",
      "If client addresses are not enabled, ask the administrator which legacy address and sender rules apply; do not invent a client address.",
    ],
  ...
  },
  {
    ...
    moduleId: "documents",
    question: "How do I resolve a client email that needs review?",
    answer: "Open Email review and verify the client, sender and matching evidence. No shipment is preselected. Review approval applies to one item and does not permanently approve the sender or bypass malware scanning.",
    steps: [
      "Open Documents, then Email review, or use Open email review on the client's address card.",
      "Select the item and check its client, sender, received time and reason. Use Preview document when a file is available.",
      "For a held email without a downloaded file, verify the sender and choose Approve email and scan attachments. Wait for scanning and processing before a shipment decision.",
      "For a stored file, compare the candidate identifiers with the document, search if needed, select the verified shipment and choose Attach document. Leave it open if the evidence is insufficient.",
      "For a wrong client, use Wrong client? and confirm Reassign, then reopen the item to choose its shipment. Discard unwanted items only after reviewing the confirmation.",
      "If the shipment has published Entry Proof, review the new draft before publishing. The customer keeps seeing the previous publication until then.",
    ],
    href: "/app/documents/inbound-review",
    actionLabel: "Open Email review",
    tags: ["email review", "unknown sender", "conflict", "shipment match", "reassign", "discard", "entry proof"],
  },
  {
    id: "manage-client-email-address",
    moduleId: "documents",
    question: "How do I manage a client's document email address and approved senders?",
    answer: "With client addresses enabled and settings management access, use Document Email to manage each destination. Review new senders stores clean files for broker review; Approved senders only holds unknown mail before download; Any sender still requires clean scans and safe shipment matching. Blocked sender rules take precedence.",
    steps: [
      "Open Manage Account, then Document Email. Find the client row, copy its current address or choose Manage.",
      "Choose the sender policy. Use Add approved sender only when the sender should be authorized for this destination beyond a single review item.",
      "Use Regenerate address to issue a new one; distribute it before the old address's 30-day grace period ends.",
      "Use Suspend to reject new emails temporarily and Resume to restore intake. Revoke permanently rejects emails to that address; read the confirmation before acting.",
      "Enable receipt confirmations only when intended. They also require deployment-level automatic replies; a missing receipt does not establish whether a file was processed.",
    ],
    href: "/app/admin/settings",
    actionLabel: "Open document settings",
    tags: ["email", "address", "sender policy", "allowlist", "rotate", "suspend", "revoke", "receipt"],
  },
  {
    id: "quarantine-document",
    moduleId: "documents",
    question: "What do I do with a quarantined email or attachment?",
    answer: "Quarantine protects the account from unknown or incorrectly routed legacy inbound mail. Client-address sender and shipment decisions use Documents → Email review instead. Review the sender, subject, client, attachment and destination before releasing a quarantined item; discard or block items that should not enter processing.",
    steps: [
