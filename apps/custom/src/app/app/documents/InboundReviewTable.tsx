...
type Review = { id: string; reason: string; clientId: string | null; client: { name: string } | null; createdAt: string; inboundEmail: { originalFromAddress: string; subject: string | null }; shipmentDocument: { id: string; fileName: string; status: string } | null; candidateSummary: { shipmentId: string; score: number; signals: { type: string; value: string }[]; reasoning?: string | null }[] | null };
  ...
  return <div className="space-y-5">
    ...
    {selected && <section aria-label="Review selected document" className="lg:sticky lg:top-4 rounded-xl border border-blue-200 bg-surface p-5 space-y-4"><div className="flex justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-wider text-brand">Review decision</p><h2 className="mt-1 text-lg font-semibold">{selected.shipmentDocument?.fileName || 'Approve this email'}</h2><p className="text-sm text-ink-muted">{selected.client?.name || 'Operations inbox'} · {parseSenderNameAndEmail(selected.inboundEmail.originalFromAddress).nameOrEmail}</p></div><button className={button} disabled={!!busy} onClick={() => setSelected(null)}>Close</button></div>
      ...
      {selected.shipmentDocument ? <>
        {!!selected.candidateSummary?.length && <div><h3 className="text-sm font-semibold">Why this shipment</h3>{selected.candidateSummary.map(c => <div key={c.shipmentId} className="mt-2 rounded-lg bg-surface-muted p-3 text-sm"><p className="font-medium">{candidateShipments.find(s => s.id === c.shipmentId)?.shipmentNumber || 'Shipment unavailable'} · {Math.round(c.score * 100)}% match</p>{!!c.signals.length && <p className="mt-0.5 text-ink-muted">{c.signals.map(s => `${s.type.replaceAll('_', ' ')} ${s.value}`.trim()).join(' · ')}</p>}{c.reasoning && <p className="mt-1 text-ink-muted italic">“{c.reasoning}”</p>}</div>)}</div>}
