...
export function InboundReviewTable() {
  ...
  const [selected, setSelected] = useState<Review | null>(null), [shipments, setShipments] = useState<Shipment[]>([]), [shipmentId, setShipmentId] = useState(''), [q, setQ] = useState('');
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try { const r = await fetch(`/api/broker/inbound-reviews/${selected.id}?q=${encodeURIComponent(q)}`, { signal: controller.signal }); if (!r.ok) throw new Error('Unable to load shipments.'); const d = await r.json(); setShipments(d.shipments); setClients(d.clients); }
  ...
  return <div className="space-y-5">
    ...
    {selected && <section aria-label="Review selected document" className="rounded-xl border border-blue-200 bg-surface p-5 space-y-4"><div className="flex justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-wider text-brand">Review decision</p><h2 className="mt-1 text-lg font-semibold">{selected.shipmentDocument?.fileName || 'Approve this email'}</h2><p className="text-sm text-ink-muted">{selected.client?.name || 'Operations inbox'} · {selected.inboundEmail.originalFromAddress}</p></div><button className={button} disabled={!!busy} onClick={() => setSelected(null)}>Close</button></div>
      ...
      {selected.shipmentDocument ? <>
        {!!selected.candidateSummary?.length && <div><h3 className="text-sm font-semibold">Matching evidence</h3>{selected.candidateSummary.map(c => <p key={c.shipmentId} className="mt-2 rounded-lg bg-surface-muted p-3 text-sm">{shipments.find(s => s.id === c.shipmentId)?.shipmentNumber || 'Suggested shipment'} · {Math.round(c.score * 100)}% match<br /><span className="text-ink-muted">{c.signals.map(s => `${s.type.replaceAll('_', ' ')} ${s.value}`).join(' · ')}</span></p>)}</div>}
        <label className="block text-sm font-medium">Find a shipment for {selected.client?.name || 'this inbox'}<input value={q} onChange={e => setQ(e.target.value)} placeholder="Search shipment number" className="mt-2 w-full rounded-lg border border-border bg-surface px-3 py-2" /></label>
        <label className="block text-sm font-medium">Attach to shipment<select value={shipmentId} onChange={e => setShipmentId(e.target.value)} className="mt-2 w-full rounded-lg border border-border bg-surface px-3 py-2"><option value="">Choose a shipment — no selection yet</option>{shipments.map(s => <option key={s.id} value={s.id}>{s.shipmentNumber} · {s.importerName}</option>)}</select></label>
