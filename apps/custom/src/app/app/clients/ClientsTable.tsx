...
function EvidenceChip({ good, label, title }: { good: boolean; label: string; title?: string }) {
  return <span title={title} className={`rounded-full px-2 py-1 font-semibold capitalize ${good ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-800"}`}>{good ? "●" : "○"} {label.replaceAll("_", " ")}</span>;
}
