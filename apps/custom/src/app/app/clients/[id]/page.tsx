...
export default function ClientSetupPage() {
    ...
    const [data, setData] = useState<Data | null>(null), [error, setError] = useState(''), [url, setUrl] = useState('');
    useEffect(() => {
        fetch(`/api/broker/clients/${id}/setup`).then(async (r) => {
            if (!r.ok)
                throw new Error('Unable to load client setup');
            return r.json();
