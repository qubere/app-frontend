...
type Address = { id: string; clientId: string | null; address: string; status: string; senderPolicy: string; autoReplyEnabled: boolean; autoAttachPolicy: string; activeKey: string | null; graceUntil: string | null; client: { name: string } | null; inboundEmails: { receivedAt: string }[] };
...
export function ClientInboundAddresses({ clientId, onNavigate }: { clientId?: string; onNavigate?: () => void }) {
  ...
  return <section className="rounded-xl border border-border bg-surface p-5 space-y-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="flex items-center gap-2 text-lg font-semibold"><Mail size={19} className="text-brand" />Send documents by email</h2><p className="mt-1 text-sm text-ink-muted">Each address identifies a client. Uncertain matches wait for broker review.</p></div><Link className={`${button} text-brand`} href={`/app/documents/inbound-review${clientId ? `?clientId=${encodeURIComponent(clientId)}` : ""}`} onNavigate={onNavigate}>Open email review →</Link></div>
    ...
    {addresses.filter(a => a.id === expanded).map(a => <div key={a.id} className="rounded-lg border border-blue-200 bg-blue-50/30 p-4 space-y-3"><h3 className="font-medium">{a.client?.name || 'Operations inbox'}</h3><label className="block text-sm">Sender policy<select className="ml-3 rounded-lg border border-border bg-surface p-2" value={a.senderPolicy} disabled={!!busy} onChange={e => mutate(a.id, 'POLICY', { senderPolicy: e.target.value })}><option value="REVIEW">Review new senders</option><option value="ALLOWLIST">Approved senders only</option><option value="OPEN">Any sender</option></select></label><p className="text-xs text-ink-muted">Review: store clean documents for review. Approved only: hold unknown senders before download. Any sender: process clean documents; ambiguous matches still need review.</p><label className="block text-sm">Auto-attach to shipment<select className="ml-3 rounded-lg border border-border bg-surface p-2" value={a.autoAttachPolicy ?? 'CONFIDENT'} disabled={!!busy} onChange={e => mutate(a.id, 'POLICY', { autoAttachPolicy: e.target.value })}><option value="OFF">Off — always review</option><option value="CONFIDENT">When confident (recommended)</option><option value="AGGRESSIVE">Aggressive — attach the best match</option></select></label><p className="text-xs text-ink-muted">When confident: attach a document automatically only when an identifier (shipment #, container, BL, PO) in the email or document resolves to exactly one shipment. Aggressive also attaches the single most likely shipment at lower confidence. A trusted sender is still required; unknown senders always go to review.</p><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={a.autoReplyEnabled} disabled={!!busy} onChange={e => mutate(a.id, 'POLICY', { autoReplyEnabled: e.target.checked })} />Send receipt confirmation when automatic replies are enabled</label>
