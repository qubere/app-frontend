...
export function renderPortalEmail(value:unknown):RenderedEmail {const p=value as {type:string;href:string};if(!p||!COPY[p.type]||!p.href.startsWith('/'))throw new Error('Invalid portal notification');const url=new URL(p.href,process.env.NEXT_PUBLIC_PORTAL_URL||'http://localhost:3002').toString();const message=COPY[p.type];return {subject:message,text:message+'\n'+url,html:`<p>${escapeHtml(message)}</p><p><a href="${escapeHtml(url)}">Open your portal</a></p>`}}
export async function queuePortalUpdate(accountId:string,clientId:string,type:string,sourceId:string,href:string) {
 if(!COPY[type])throw new Error('Unknown portal notification');
 ...
 for(const recipient of recipients){
  const prefs=(recipient.notifyPrefs??{}) as Record<string,unknown>;if(prefs[type]===false)continue;
  ...
  const assignment=await db.userClientAssignment.findUnique({where:{userId_clientId:{userId:recipient.userId!,clientId}},select:{id:true}});if(!membership||!assignment)continue;
  ...
  await db.$transaction(async tx=>{
   ...
   if(!current.bellDeliveredAt && prefs.portal!==false){await tx.notification.create({data:{accountId,userId:recipient.userId!,type,message:COPY[type],entityType:'PortalUpdate',entityId:clientId}});await tx.complianceNotification.update({where:{id:queued.id},data:{bellDeliveredAt:new Date()}});}
  });
 }
}
 ...
 */
export async function syncPortalNotifications(){return runWithDataMode(null,()=>runWithAccountId(null,async()=>{
 const accounts=await db.clientStakeholder.findMany({where:{loginStatus:'ACTIVE'},select:{accountId:true},distinct:['accountId']});
 for(const {accountId} of accounts)await runWithAccountId(accountId,async()=>{
  ...
  const previous=(checkpoint?.metadata as {through?:string}|null)?.through;const since=previous?new Date(previous):new Date(0);const through=new Date();const window={gt:since,lte:through};
  ...
  for(const p of proofs)await queuePortalUpdate(accountId,p.clientId,'ENTRY_PROOF_PUBLISHED',p.id,`/entries/${p.filingId}`);
  for(const e of etas)if(e.shipment.clientId)await queuePortalUpdate(accountId,e.shipment.clientId,'ETA_CHANGED',e.id,`/shipments/${e.shipmentId}`);
  for(const h of holds)if(h.shipment.clientId)await queuePortalUpdate(accountId,h.shipment.clientId,'HOLD_PLACED',h.id,`/shipments/${h.shipmentId}`);
  for(const f of filings)if(f.shipment?.clientId)await queuePortalUpdate(accountId,f.shipment.clientId,'CUSTOMS_RELEASED',f.id,`/shipments/${f.shipmentId}`);
  for(const r of requests)await queuePortalUpdate(accountId,r.clientId,'DOCUMENT_REQUESTED',r.id,`/requests/${r.id}`);
  for(const i of invoices)if(i.clientId)await queuePortalUpdate(accountId,i.clientId,'INVOICE_ISSUED',i.id,'/invoices');
  for(const c of cases)if(c.clientId){await syncClientSetup(accountId,c.clientId);if(c.activatedAt)await queuePortalUpdate(accountId,c.clientId,'ACCOUNT_ACTIVATED',c.id,'/setup');}
  for(const p of poas)if(p.importerOfRecord.clientId){await syncClientSetup(accountId,p.importerOfRecord.clientId);await queuePortalUpdate(accountId,p.importerOfRecord.clientId,'POA_SIGNED',p.id,'/setup');}
  for(const f of forms)if(f.case?.clientId){await syncClientSetup(accountId,f.case.clientId);await queuePortalUpdate(accountId,f.case.clientId,'FORM_5106_ACCEPTED',f.id,'/setup');}
