...
export const CRON_JOB_DEFINITIONS: CronJobDefinition[] = [
  ...
  {
    id: "license-alerts",
    schedulerName: "qubere-license-alerts",
    name: "License Management Alerts",
    description: "Computes and emails License Management alerts (expiring licenses + utilization thresholds) for every account with license alerting enabled.",
    cronExpression: "0 8 * * *",
    scheduleLabel: "Daily at 08:00 UTC",
    trigger: { kind: "http", path: "/api/cron/license-alerts", method: "GET" },
  },
  {
    id: "inbound-email-processing",
    schedulerName: "qubere-inbound-email-processing",
    name: "Inbound Email Processing Backstop",
    description: "Durable backstop that re-runs the idempotent inbound-email worker tick, guaranteeing eventual processing if the webhook's inline dispatch never finishes.",
    cronExpression: "*/5 * * * *",
    scheduleLabel: "Every 5 minutes / Backstop",
    trigger: { kind: "http", path: "/api/cron/inbound-email-processing", method: "GET" },
  },
  {
    id: "report-cleanup",
    schedulerName: "qubere-report-cleanup",
    name: "Report Artifact Retention Cleanup",
    description: "Deletes expired ReportArtifact storage objects and marks them removed once past their retention window.",
    cronExpression: "30 4 * * *",
    scheduleLabel: "Daily at 04:30 UTC",
    trigger: { kind: "http", path: "/api/cron/report-cleanup", method: "GET" },
  },
  {
    id: "adcvd-monitor",
    schedulerName: "qubere-adcvd-monitor",
    name: "AD/CVD Monitoring Sweep",
    description: "Executes continuous AD/CVD monitoring sweep over all active orders, checking for status changes.",
    cronExpression: "0 7 * * *",
  ...
  },
  {
    ...
    name: "Bond Monitoring Sweep",
    description: "Executes continuous customs bond monitoring sweep, checking for expiring or lapsed bonds.",
    cronExpression: "0 9 * * *",
  ...
  {
    id: "report-schedules",
    schedulerName: "qubere-report-schedules",
    name: "Report Schedule Dispatcher",
    description: "Dispatches due scheduled compliance reports (DAILY/WEEKLY/MONTHLY) queued for generation and delivery.",
    cronExpression: "0 * * * *",
    scheduleLabel: "Hourly",
