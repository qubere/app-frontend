...
type Context = { accountId: string; userId: string };
...
export async function buildEntryProofPayload(filingId: string, ctx: Context): Promise<EntryProofPayload> {
  ...
  if (!filing?.shipment?.clientId) throw new Error('PROOF_REQUIRES_CLIENT_SHIPMENT');
  if (filing.country && filing.country !== 'US') throw new Error('PROOF_REQUIRES_US_ENTRY');
  ...
  if (!lines.length) throw new Error('PROOF_REQUIRES_ENTRY_LINES');
  ...
  const tariff = computeFilingTariff(lines, {}, rates);
  ...
  const potentialAssists = Array.isArray(assists?.potentialAssists) ? assists.potentialAssists as Array<{declared?: boolean; estimatedValue?: number}> : [];
  ...
  const proofLines: ProofLineInput[] = lines.map((l,i) => {
    ...
    allocatedMpf = allocatedMpf.plus(mpf); allocatedHmf = allocatedHmf.plus(hmf);
    ...
    const dutyStack: DutyStackRow[] = [
      ...
      { key:'HMF',label:'Harbor maintenance fee',status:'EVALUATED_APPLICABLE',ratePct:0.125,amountUsd:hmf.toNumber(),detail:null },
    ];
...
export async function generateEntryProof(filingId: string, ctx: Context) {
  ...
  return db.$transaction(async tx => {
    ...
    if(draft) await tx.entryProof.update({where:{id:draft.id},data:{status:'SUPERSEDED'}});
    ...
    if(draft) await tx.entryProof.update({where:{id:draft.id},data:{supersededById:proof.id}});
    return proof;
  });
}
...
export async function publishEntryProof(filingId: string, ctx: Context) {
  ...
  if(!existing) await generateEntryProof(filingId,ctx);
  return db.$transaction(async tx => {
    await tx.$queryRaw`SELECT id FROM "CustomsFiling" WHERE id = ${filingId} AND "accountId" = ${ctx.accountId} FOR UPDATE`;
    const draft = await tx.entryProof.findFirst({where:{filingId,accountId:ctx.accountId,status:'DRAFT'},orderBy:{version:'desc'}});
    if(!draft) throw new Error('PROOF_DRAFT_CHANGED_RETRY');
    const previous = await tx.entryProof.findFirst({where:{filingId,accountId:ctx.accountId,status:'PUBLISHED'}});
    if(previous) { await tx.entryProof.update({where:{id:previous.id},data:{status:'SUPERSEDED',supersededById:draft.id}}); await tx.entryProofEvent.create({data:{entryProofId:previous.id,accountId:ctx.accountId,type:'SUPERSEDED',actorType:'BROKER',actorUserId:ctx.userId,detail:{supersededById:draft.id}}}); }
    const now = new Date();
