 ...
 * compliance trail in `@/lib/audit`). Every log line is a single JSON object
 * so it stays greppable and parseable by a log aggregator, with a consistent
 * set of fields across every call site: timestamp, level, accountId, userId.
 */
 ...
 * `withCronRoute` in `@/lib/api/auth-guards`, so any route built on those
 * gets this automatically without a per-route change.
 */
