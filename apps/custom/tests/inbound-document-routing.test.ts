...
const doc = { id: 'doc', clientId: 'target', shipmentId: null, status: 'Received', inboundDocumentReview: null, inboundAttachment: { inboundEmail: { id: 'email', inboundAddressId: 'address', subject: 'SHP-TGT-2026-001' } } };
beforeEach(() => { vi.resetAllMocks(); db.$transaction.mockImplementation(fn => fn(db)); db.shipmentDocument.findFirst.mockResolvedValue(doc); db.shipment.findFirst.mockResolvedValue({ id: 'shipment' }); db.customsFiling.findMany.mockResolvedValue([]); db.inboundDocumentReview.upsert.mockResolvedValue({ id: 'review' }); match.mockResolvedValue({ matchedShipmentId: null, candidates: [] }); });
describe('recipient-owned document decisions', () => {
  ...
  it('keeps unknown-sender documents out of automatic selection', async () => {
    db.shipmentDocument.findFirst.mockResolvedValue({ ...doc, inboundDocumentReview: { reason: 'UNKNOWN_SENDER', status: 'OPEN' } });
    await routeParsedInboundDocument('a', 'doc', 'invoice');
