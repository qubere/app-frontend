...
async function run() {
  ...
  // Wait to see if we get redirected to Clerk login
  ...
  if (isLoginPage) {
    console.log("Clerk login page detected. Attempting login...");
    const emailInput = page.locator('input[type="email"], input[name="identifier"]');
    await emailInput.fill("owner.acme@qubere.ai");
    ...
    console.log("Already authenticated or on workspace page.");
  }
  ...
  // Wait for 10 seconds to ensure chat renders and DB queries complete
  console.log("Waiting for rendering components...");
  await page.waitForTimeout(10000);

  // Take a screenshot of the loaded chat page and save to artifacts folder
  const screenshotPath = "/Users/rachitlohani/.gemini/antigravity-ide/brain/05ec099a-a361-4628-9185-d0f3bacf10cc/chat_page_screenshot.png";
