...
describe('Onboarding client ownership', () => {
  ...
  it('resolves and links a Party for the new entity (#320 Phase 1)', async () => {
    await entity.POST(new Request('http://custom/api/onboarding/cases/case/entities', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ importerNumberType: 'EIN', importerNumber: '123', legalName: 'Target', entityType: 'CORPORATION', addressLine1: 'One St', city: 'City', postalCode: '12345' }) }));
    expect(resolvePartyForCompany).toHaveBeenCalledWith(
      { accountId: 'broker', userId: 'staff', requestId: 'test' },
