...
interface RawExtractionModalProps extends Omit<DocumentReviewPanelProps, "onClose" | "titleId"> {
  isOpen: boolean;
  onClose: () => void;
}
...
export function RawExtractionModal({ isOpen, onClose, fileName, ...panelProps }: RawExtractionModalProps) {
  return (
    <Modal isOpen={isOpen} onClose={onClose} titleId={TITLE_ID} size="lg" className="max-h-[90vh] flex flex-col space-y-0">
