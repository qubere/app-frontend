...
import type { AgentInvocation } from "@/lib/tmsAgentInvocations";
    ...
    const poll = async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (cancelledRef.current || isStopped) return;
          hasActiveRun = data?.status === "PROCESSING" || data?.status === "PENDING";
        }
      } catch {
  ...
  return (
    <div className="space-y-3">
      {invocations.map((inv) => {
        ...
        const style = STATUS_STYLES[inv.status] || STATUS_STYLES.COMPLETED;
        const waterfallSpan = Math.max(inv.totalDurationMs, 1);

        return (
          <div key={inv.runId} className="rounded-2xl border border-border bg-white overflow-hidden shadow-2xs">
            <button
              type="button"
            ...
            {isExpanded && (
              <div className="border-t border-border bg-[#FAFAFC] px-4 py-4 space-y-2.5">
                {inv.steps.map((step, idx) => {
                  const stepStyle = STATUS_STYLES[step.status] || STATUS_STYLES.COMPLETED;
                  ...
                  return (
                    <div key={step.id} className="space-y-2">
                      <div
                        ...
                        title="Click to inspect step input/output snapshots"
                      >
                      ...
                      {isStepExpanded && (
                        <div className="ml-8 p-3.5 bg-white rounded-xl border border-border space-y-3 text-xs shadow-2xs">
