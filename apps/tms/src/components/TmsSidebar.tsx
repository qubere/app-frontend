...
export function TmsSidebar({
  currentAccountId,
  accountName = "Enterprise Freight",
  ...
  const fetchExceptions = useCallback(async () => {
    try {
      const res = await fetch("/api/exceptions/count", { cache: "no-store" });
      if (!res.ok) return;
      const data = await res.json();
      setPendingExceptionsCount(typeof data.count === "number" ? data.count : 0);
    } catch {
      // Silent poll fallback
    }
  }, []);

  usePolling(fetchExceptions, 60_000);
  ...
  return (
    <>
      ...
      >
        ...
        {/* Ask Qubere AI Launcher */}
        <div className={cn("px-3", collapsed ? "mt-4" : "mt-4")}>
          <Link
            href="/chat"
            onClick={() => setMobileOpen(false)}
            title={collapsed ? (labels.askQubere ?? "Ask Qubere AI") : undefined}
            className={cn(
          ...
          >
            ...
            {!collapsed && (
              <span className="text-sm font-semibold truncate">{labels.askQubere ?? "Ask Qubere AI"}</span>
            )}
          </Link>
        </div>
                    ...
                    return (
                      <div key={rowIndex} className="flex gap-1.5">
                        {rowItems.map((item) => {
                          ...
                          const badgeCount = item.id === "exceptions" ? pendingExceptionsCount : (item.badge ?? 0);
                          return (
                            <Link
                              key={item.id}
                              href={item.href}
        ...
        <div className="p-3 border-t border-border bg-white/40">
          {!collapsed && (
            <div className="px-2 pb-2 text-xs text-ink-muted">
              <p className="font-semibold text-ink">Qubere AI Freight OS</p>
      ...
      {/* Floating Ask Qubere launcher */}
      <a
        href="/chat"
        target="_blank"
        rel="noopener noreferrer"
        title={labels.askQubere ?? "Ask Qubere AI"}
        aria-label={labels.askQubere ?? "Ask Qubere AI"}
      ...
      >
        ...
        <span className="pointer-events-none absolute right-full mr-3 whitespace-nowrap rounded-lg bg-ink text-white text-xs font-medium px-2.5 py-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
          {labels.askQubere ?? "Ask Qubere AI"}
        </span>
      </a>
    </>
  );
