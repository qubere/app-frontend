...
export interface AnswerCard {key:string;question:string;answer:string;status:'OK'|'ATTENTION'|'ACTION_REQUIRED'|'UNKNOWN';facts:{label:string;value:string;href?:string}[];updatedAt:string;askHref?:string}
export interface ShipmentAnswerSet {
 shipmentId:string;shipmentNumber:string;generatedAt:string;
 ...
 cost:{dutyAndFeesUsd:number|null;brokerChargesUsd:number|null;estimatedTotalUsd:number|null;costIsPartial:boolean;invoices:{id:string;number:string;status:string;total:number;currency:string}[]};
 cards:AnswerCard[];milestones:{label:string;at:string|null;location:string|null}[];referenceNumbers:{label:string;value:string}[];
 needsFromYou:{id:string;label:string;dueAt:string|null;href:string}[];
}
export interface ShipmentAnswersInput {
 id:string;shipmentNumber:string;generatedAt:string;transportationStatus:string;customsStatus:string;promiseState:string|null;healthLabel:string|null;carrierName:string|null;portOfEntry:string|null;estimatedArrival:string|null;lastFreeDay:string|null;demurrageExposureUsd:number|null;
 eta?:{current:string;previous:string|null;changedOn:string;reasonCode:string|null;confidence:number|null}|null;
 proofs:{dutyAndFeesUsd:number;complete:boolean}[];
 charges:{netAmount:number;currency:string;portalVisible:boolean;status:string}[];
 invoices:{id:string;number:string;status:string;total:number;currency:string}[];
 milestones:{label:string;at:string|null;location:string|null}[];
 referenceNumbers:{label:string;value:string}[];
 requests:{id:string;title:string;status:string;dueAt:string|null}[];
 deadlines:{id:string;customerActionable:boolean;customerLabel:string|null;status:string;dueAt:string|null}[];
 holds:{agencyCode:string;status:string}[];
}
...
export function assembleShipmentAnswers(i:ShipmentAnswersInput):ShipmentAnswerSet {
 ...
 const partial=duty===null || broker===null || i.proofs.some(p=>!p.complete) || visibleCharges.some(c=>c.currency!=='USD');
 ...
 const needs=[...i.requests.filter(r=>['OPEN','CUSTOMER_RESPONDED'].includes(r.status)).map(r=>({id:r.id,label:r.title,dueAt:r.dueAt,href:`/requests/${r.id}`})),...i.deadlines.filter(d=>d.customerActionable&&d.status==='OPEN').map(d=>({id:d.id,label:d.customerLabel||'Your broker needs information',dueAt:d.dueAt,href:`/shipments/${i.id}`}))];
 ...
 if(i.lastFreeDay)cards.push({key:'free-time',question:'When does free time end?',answer:`Last free day: ${i.lastFreeDay.slice(0,10)}.`,status:new Date(i.lastFreeDay).getTime()-new Date(i.generatedAt).getTime()<48*3600000?'ATTENTION':'OK',facts:i.demurrageExposureUsd!==null?[{label:'Estimated demurrage exposure',value:`$${i.demurrageExposureUsd.toFixed(2)} USD`}]:[],...common});
