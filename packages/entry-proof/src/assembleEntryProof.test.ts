...
describe('proof score and safety contract', () => {
 ...
 it.each(['NOT_EVALUATED','DATA_UNAVAILABLE','REVIEW_REQUIRED'] as const)('does not equate %s to zero duty',status=>{
  const l=line();l.dutyStack.push({key:'ANTIDUMPING',label:'AD',status,ratePct:null,amountUsd:0,detail:null});
 ...
 it('sums identified savings with decimal arithmetic and deduplicates findings',()=>{
  const f=flagCopy({id:'find',rule:'x',severity:'Info',dutyImpactUsd:-0.2}); const i=input([line({flags:[f]})]);i.findings=[f];i.refundOpportunities=[{status:'Identified',estimatedRefundAmount:0.1},{status:'Claimed',estimatedRefundAmount:999}];
 ...
 it('drops internal properties at all object boundaries and unknown finding copy',()=>{
  const f=flagCopy({rule:'SECRET recommendation',severity:'High'}); const i=input([line()]);
  Object.assign(i,{grossProfit:123});Object.assign(i.lines[0],{humanNotes:'SECRET'});Object.assign(i.lines[0].valuation,{actualBuyCost:456});Object.assign(f,{recommendation:'SECRET'}); i.findings=[f];
  const json=JSON.stringify(assembleEntryProof(i));expect(json).not.toMatch(/SECRET|grossProfit|humanNotes|actualBuyCost|recommendation/);
 });
});
