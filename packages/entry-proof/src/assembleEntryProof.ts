...
export interface AssembleEntryProofInput extends Omit<EntryProofPayload, 'schemaVersion' | 'scorecard' | 'lines'> {
  ...
  refundOpportunities: { status: string; estimatedRefundAmount: number }[];
}
export function assembleEntryProof(input: AssembleEntryProofInput): EntryProofPayload {
  ...
  const lines: EntryProofLine[] = input.lines.map(l => {
    ...
    if (l.classificationStatus === 'missing') reasons.push('Classification is missing.');
    if (majorUnapproved) reasons.push('Classification approval is needed for a material entry line.');
    if (missingBase) reasons.push('A published base duty rate is unavailable.');
    if (high) reasons.push('An open high-priority finding needs review.');
    if (l.dutyStack.some(d => d.status === 'REVIEW_REQUIRED')) reasons.push('A duty measure requires review.');
    const atRisk = reasons.length > 0;
    if (!atRisk) {
      if (l.classificationStatus === 'sourced_unapproved') reasons.push('Classification awaits approval.');
      if (l.dutyStack.some(d => ['NOT_EVALUATED','DATA_UNAVAILABLE'].includes(d.status))) reasons.push('Some duty measures have not been fully evaluated.');
      if (l.flags.some(f => f.severity === 'WARNING')) reasons.push('An open finding needs review.');
      if (l.htsConfidence !== null && l.htsConfidence < 75) reasons.push('Classification confidence is below 75%.');
    }
    return {
