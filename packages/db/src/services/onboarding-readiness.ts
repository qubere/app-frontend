...
export function computeReadiness(c: CaseShape): ReadinessResult {
  ...
  const poaOk = c.entities.length > 0 && c.entities.every((e) => e.poa?.status === "executed");
  ...
  const bondOk = c.entities.length > 0 && c.entities.every((e) => {
  ...
  // 5 — Screening cleared
  const screeningOk = c.entities.length > 0 && c.entities.every(
