...
export async function syncClientSetup(accountId: string, clientId: string) {
    ...
    if (c.contactEmail)
        await syncClientStakeholder({ accountId, clientId, email: c.contactEmail, name: c.contactName || c.contactEmail, role: 'CUSTOMS_CONTACT', sourceEvent: 'CLIENT_CONTACT' });
    if (c.billingContactEmail)
        await syncClientStakeholder({ accountId, clientId, email: c.billingContactEmail, name: c.billingContactName || c.billingContactEmail, role: 'BILLING_CONTACT', sourceEvent: 'CLIENT_CONTACT' });
        ...
        for (const e of onboarding.entities) {
            ...
            for (const raw of officers) {
                ...
                if (typeof o.email === 'string')
                    await syncClientStakeholder({ accountId, clientId, email: o.email, name: typeof o.name === 'string' ? o.name : o.email, title: typeof o.title === 'string' ? o.title : null, role: 'OFFICER_SIGNER', isSigner: true, onboardingEntityId: e.id, sourceEvent: 'ONBOARDING_OFFICER' });
            }
            if (e.bond && ['verified', 'attested'].includes(e.bond.status)) {
                ...
                if (docId) {
                    const doc = await db.shipmentDocument.findFirst({ where: { id: docId, accountId, clientId }, select: { fileUrl: true } });
                    if (doc?.fileUrl)
                        await promoteClientDocument({ accountId, clientId, kind: 'BOND', title: `Bond ${e.bond.bondNumber}`, storageUrl: doc.fileUrl, sourceModel: 'Bond', sourceId: e.bond.id, effectiveDate: e.bond.effectiveDate, expirationDate: e.bond.expirationDate });
                }
            }
        }
        for (const r of onboarding.fiveOhSixRecords) {
            if (r.status === 'accepted' && r.pdfDocumentUrl)
                await promoteClientDocument({ accountId, clientId, kind: 'FORM_5106', title: 'Accepted CBP Form 5106', storageUrl: r.pdfDocumentUrl, sourceModel: 'FiveOhSixRecord', sourceId: r.id, effectiveDate: r.acceptedAt });
        }
    }
    for (const p of poas.values()) {
        if (p.status === 'executed') {
            ...
            if (url)
                await promoteClientDocument({ accountId, clientId, kind: 'EXECUTED_POA', title: 'Executed Power of Attorney', storageUrl: url, sourceModel: 'PowerOfAttorney', sourceId: p.id, effectiveDate: p.signedDate, expirationDate: p.expirationDate });
            const email = p.signerEmail || p.envelope?.signerEmail;
            if (email)
                await syncClientStakeholder({ accountId, clientId, email, name: p.signerName || p.envelope?.signerName || email, title: p.signerTitle, role: 'OFFICER_SIGNER', isSigner: true, sourceEvent: 'POA_SIGNED' });
        }
        else if (['revoked', 'expired'].includes(p.status))
            await db.clientDocument.updateMany({ where: { accountId, clientId, sourceModel: 'PowerOfAttorney', sourceId: p.id }, data: { status: 'REVOKED', portalVisible: false } });
    }
}
