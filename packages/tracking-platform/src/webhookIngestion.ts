...
export interface TrackingSignalPersistedInput {
  accountId: string;
  shipmentId: string;
...
export type TrackingSignalPersistedHook = (input: TrackingSignalPersistedInput) => Promise<unknown>;
...
export interface TrackingWebhookIngestionDependencies {
  ...
  now?: () => Date;
  onSignalPersisted?: TrackingSignalPersistedHook;
}
...
export async function recordTrackingWebhookFailure(
  connectionKey: string,
  error: unknown,
  dependencies: Pick<TrackingWebhookIngestionDependencies, "dbClient" | "now"> = {}
) {
  const client = dependencies.dbClient ?? db;
  const connection = await (client as any).integrationConfig.findFirst({
...
export async function ingestTrackingWebhook(
  ...
  const onSignalPersisted = dependencies.onSignalPersisted;
  ...
  for (const signal of signals) {
    ...
    if (onSignalPersisted) {
      await onSignalPersisted({
        accountId: connection.accountId,
        shipmentId: signal.shipmentId!,
        trackingEventId: persisted.trackingEvent.id,
        canonicalEventType: persisted.trackingEvent.eventType,
