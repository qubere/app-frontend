...
SECRET_BINDINGS="DATABASE_URL=${DATABASE_URL_SECRET}:latest,DIRECT_URL=${DIRECT_URL_SECRET}:latest,CLERK_SECRET_KEY=${CLERK_SECRET_KEY_SECRET}:latest,GEMINI_API_KEY=${GEMINI_API_KEY_SECRET}:latest,CRON_SECRET=${CRON_SECRET_SECRET}:latest,DOCLING_API_KEY=${DOCLING_API_KEY_SECRET}:latest"
for secret in "${DATABASE_URL_SECRET}" "${DIRECT_URL_SECRET}" "${CLERK_SECRET_KEY_SECRET}" "${GEMINI_API_KEY_SECRET}" "${CRON_SECRET_SECRET}" "${DOCLING_API_KEY_SECRET}"; do
  gcloud secrets describe "${secret}" --project="${GCP_PROJECT_ID}" >/dev/null || { echo "Missing secret: ${secret}" >&2; exit 1; }
done

BASE_ENV="APP_ENV=demo,NEXT_PUBLIC_APP_ENV=demo,NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY=${NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY},STORAGE_PROVIDER=gcs,GCS_BUCKET=${GCS_BUCKET},DOCUMENT_PARSER_PROVIDER=ibm-docling,DOCLING_API_BASE_URL=${DOCLING_API_BASE_URL},DOCUMENT_MALWARE_SCAN_MODE=basic,CUSTOMS_FILING_MOCK_RESPONSES=true"
