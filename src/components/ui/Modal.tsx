...
export interface ModalProps {
  isOpen: boolean;
  onClose: () => void;
  /** Must match the id on the element carrying the dialog title (see ModalHeader). */
  titleId: string;
 ...
 */
export function Modal({ isOpen, onClose, titleId, closeDisabled, size = "md", className, children }: ModalProps) {
  ...
  return (
    ...
    >
      <div
        ...
        className={cn(
          "bg-white rounded-3xl border border-border shadow-2xl w-full p-6 space-y-5 animate-in fade-in zoom-in-95 duration-200",
          sizeClasses[size],
          className
        )}
      >
